...
angular.module('copayApp.services').factory('bitpayAccountService', function($log, lodash, platformInfo, appIdentityService, bitpayService, bitpayCardService, storageService, gettextCatalog, popupService) {
   ...
   *   "To {{reason}} you must pair this app with your DigiByte Foundation account ({{email}})."
   *
  ...
  root.pair = function(pairData, pairingReason, cb) {
    checkOtp(pairData, function(otp) {
      ...
      bitpayService.postAuth(json, function(data) {
        ...
        $log.info('DigiByte Foundation service BitAuth create token: SUCCESS');

        fetchBasicInfo(apiContext, function(err, basicInfo) {
          if (err) return cb(err);
          var title = gettextCatalog.getString('Add DigiByte Foundation Account?');
          var msg;

          if (pairingReason) {
            msg = gettextCatalog.getString('To {{reason}} you must first add your DigiByte Foundation account - {{email}}', {
              reason: pairingReason,
              email: pairData.email
            });
          } else {
            msg = gettextCatalog.getString('Add this DigiByte Foundation account ({{email}})?', {
              email: pairData.email
            });
          }
          ...
          popupService.showConfirm(title, msg, ok, cancel, function(res) {
          	if (res) {
                ...
                familyName: basicInfo.familyName
  				    ...
  				    $log.info('User cancelled DigiByte Foundation pairing process');
  		        return cb(null, false);
          	}
          });
        });
      }, function(data) {
        return cb(_setError('DigiByte Foundation service BitAuth create token: ERROR ', data));
	    });
	  });
  };
  ...
  var checkOtp = function(pairData, cb) {
    if (pairData.otp) {
      var msg = gettextCatalog.getString('Enter Two Factor for your DigiByte Foundation account');
  ...
  var fetchBasicInfo = function(apiContext, cb) {
    ...
    bitpayService.post('/api/v2/' + apiContext.token, json, function(data) {
      ...
      $log.info('DigiByte Foundation Account Get Basic Info: SUCCESS');
      return cb(null, data.data.data);
    }, function(data) {
      return cb(_setError('DigiByte Foundation Account Error: Get Basic Info', data));
    });
  };
